...
string correctWord(const string& word, const vector<string>& dictionary, const bool &flag) {
    ...
    {
        for (const string& dictWord : dictionary) {
            int distance = alg_lev_optimized(word, dictWord);
            if (distance < minDistance) {
                minDistance = distance;
                correctedWord = dictWord;
            }
...
void processWords(const vector<string>::iterator& begin,
                   ...
                   mutex& resultMutex, const bool &flag) 
{
    for (auto it = begin; it != end; ++it) {
        string correctedWord = correctWord(*it, dictionary, flag);
...
void multi_thread(const string &input_text, const vector<string> &dictionary, int num_threads, const bool &flag) 
{
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        threads.emplace_back(processWords, words.begin() + start, words.begin() + end,
                             ref(dictionary), ref(correctedWords), ref(resultMutex), ref(flag));
    }
